...
App({
    ...
    queryMealRecords() {
        ...
        const fetchPage = (skip) => {
            return db.collection('meal_records')
                ...
                .then(res => {
                    ...
                    // 累加当前页数据
                    allRecords = [...allRecords, ...currentPageData];

                    console.log(`已获取${allRecords.length}条记录，当前页返回${currentPageData.length}条`);

                    // 如果当前页数据等于pageSize，说明可能还有更多数据，继续查询下一页
                    if (currentPageData.length === pageSize) {
                        return fetchPage(skip + pageSize); // 递归查询下一页
                    } else {
                        // 所有数据查询完成，返回全部记录
                        return allRecords;
                    }
        ...
        return fetchPage(0)
            .then(allData => {
                console.log('所有记录查询完成，共', allData.length, '条');
                this.globalData.fullMealRecords = allData;
                ...
                allData.forEach(record => {
                    ...
                    // 按时间排序
                    mealRecords[record.date].sort((a, b) => a.localeCompare(b));
                });

                this.globalData.mealRecords = mealRecords;
                console.log('格式化后的mealRecords:', mealRecords);
                this.emit('mealRecordsUpdated', mealRecords);
                return allData;
            })
            .catch(err => {
                console.error('分页查询用餐记录失败', err);
                return Promise.reject(err);
            });
    },
